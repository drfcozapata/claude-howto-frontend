<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-6 sm:py-12 pb-0">
    <div class="container mx-auto px-4 sm:px-6 py-4 sm:py-8">
      <!-- Breadcrumbs -->
      <nav class="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-6 overflow-x-auto">
        <RouterLink to="/" class="hover:text-primary-600 dark:hover:text-primary-400 whitespace-nowrap">Inicio</RouterLink>
        <ChevronRight class="w-3 h-3 sm:w-4 sm:h-4 shrink-0" />
        <RouterLink :to="breadcrumbParent.path" class="hover:text-primary-600 dark:hover:text-primary-400 whitespace-nowrap">{{ breadcrumbParent.name }}</RouterLink>
        <ChevronRight class="w-3 h-3 sm:w-4 sm:h-4 shrink-0" />
        <span class="text-gray-900 dark:text-white font-medium truncate">{{ chapter?.title }}</span>
      </nav>

      <!-- Chapter Header -->
      <div class="card p-4 sm:p-6 lg:p-8 mb-4 sm:mb-8">
        <div class="flex flex-col sm:flex-row items-start justify-between gap-4 mb-4">
          <div class="flex-1">
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
              <span class="px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400">
          ...
          <div class="text-right shrink-0">
            <div class="flex items-center gap-1 sm:gap-2 text-gray-500 dark:text-gray-400 mb-2">
              <Clock class="w-4 h-4 sm:w-5 sm:h-5" />
              <span class="text-sm">{{ chapter?.timeEstimate }}</span>
            </div>
        ...
        <div class="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">Progreso del módulo</span>
            <span class="text-xs sm:text-sm font-medium text-primary-600 dark:text-primary-400">{{ chapterProgress }}%</span>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-4 xl:col-span-3">
          <div class="card sticky top-20 overflow-hidden">
            <div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
              <h2 class="font-semibold text-sm sm:text-base text-gray-900 dark:text-white flex items-center gap-2">
                <List class="w-4 h-4 sm:w-5 sm:h-5" />
                Contenido
              </h2>
            </div>
            <div class="p-2 max-h-[calc(100vh-200px)] sm:max-h-[calc(100vh-240px)] overflow-y-auto scrollbar-thin">
              <button
                v-for="file in chapter?.files"
                :key="file.id"
                @click="selectFile(file.id)"
                class="w-full flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left"
                :class="{
              ...
              >
                ...
                <span class="text-xs sm:text-sm flex-1 truncate">{{ file.title }}</span>
                <CheckCircle v-if="isFileCompleted(file.id)" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500 shrink-0" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-8 xl:col-span-9">
          <div class="card min-h-[50vh] lg:min-h-150">
            <!-- Loading State -->
            ...
            <div v-else class="h-full">
              <!-- File Header -->
              <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 sm:mb-6 pb-3 sm:pb-6 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 pt-4 sm:pt-6">
                <h2 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">{{ currentFileTitle }}</h2>
              ...
              <!-- Markdown Content -->
              <div
                class="markdown-content prose dark:prose-invert max-w-none px-4 sm:px-6 pb-4 sm:pb-6 overflow-y-auto"
                style="max-height: calc(100vh - 280px);"
                v-html="renderedContent"
              ></div>

              <!-- Navigation Footer -->
              <div class="flex flex-col sm:flex-row justify-between gap-3 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700 px-4 sm:px-6 pb-4 sm:pb-6">
                <button
                  v-if="prevFile"
                  @click="selectFile(prevFile.id)"
                  class="flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:underline text-sm sm:text-base"
                >
                  <ArrowLeft class="w-4 h-4 sm:w-5 sm:h-5" />
                  <span class="truncate">{{ prevFile.title }}</span>
                </button>
                <div v-else class="hidden sm:block"></div>
                <button
                  v-if="nextFile"
                  @click="selectFile(nextFile.id)"
                  class="flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:underline text-sm sm:text-base self-start sm:self-auto"
                >
                  <span class="truncate">{{ nextFile.title }}</span>
